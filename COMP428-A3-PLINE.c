    ...
    for(k = 0; k < n; k++)
    {
        ...
        if(point[1] == dim_needed)
        {
            ...
            for(i=0; i < pn; i++)
            {
                ...
                MPI_Isend(rowbuffer, subsection_size[point[0]], MPI_INT, i, k, col_comm, &request);
            }
        }
        ...
        if(point[0] == dim_needed)
        {
            ...
            for(i=0; i < pn; i++)
            {
                ...
                MPI_Isend(colbuffer, subsection_size[point[1]], MPI_INT, i, k, row_comm, &request);
            }
        }
...
//        MPI_Bcast(colbuffer, subsection_size[point[1]], MPI_INT, dim_needed, row_comm);
        if(point[1] != dim_needed)
        {
            MPI_Recv(rowbuffer, subsection_size[point[0]], MPI_INT, dim_needed, k, col_comm, MPI_STATUS_IGNORE);
        }

        if(point[0] != dim_needed)
        {
            MPI_Recv(colbuffer, subsection_size[point[1]], MPI_INT, dim_needed, k, row_comm, MPI_STATUS_IGNORE);
        }
